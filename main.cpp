                   ...
                   int* similar_images)
{
	...
	spLoggerDestroy();
        ...
        if(all_points)
            spPointArrayDestroy(all_points);       
        free(img_near_cnt);
        free(similar_images);
}
...
int main(int argc, char* argv[]) {
	...
	if ((log_msg = spLoggerCreate(NULL, SP_LOGGER_WARNING_ERROR_LEVEL))
			!= SP_LOGGER_SUCCESS) {
		finishProgram(config, bpq, pc, kdtree, all_points, img_near_cnt, similar_images);
		return ERROR;
	}
	...
	if (log_msg != SP_LOGGER_SUCCESS) {
        ...
        all_points = extractImagesFeatures(&all_points_size,
                                           config,
                                           &log_msg,
                                           &conf_msg);

        kdarray = spKDArrayCreate(config,
                                  all_points,
                                  all_points_size,
                                  &log_msg,
                                  &conf_msg);

        kdtree = spKDTreeCreate(kdarray,
                                config,
                                &conf_msg,
                                &log_msg);
	...
	SPPoint* arr = extractImagesFeatures(&totalNumOfFeatures, config, &log_msg,
        ...
        while(1) {
            ...
            for(i=0;i<q_numOfFeats;i++){
                spBPQueueClear(bpq);
                knn = findKNearestNeighbors(kdtree,
                                            bpq,
                                            q_features[i]); 
            ...
            for(i=0;i<numOfSimilarImages;i++){
                printf("%d%c",similar_images[i] ,i+1 == numOfSimilarImages ? '\n' : ' '); 
            }            

            //re-initializing query-related resources
            spPointArrayDestroy(q_features);
