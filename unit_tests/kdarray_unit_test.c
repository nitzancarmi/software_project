...
int init_test(SPConfig attr) {
	...
	int A[2][5] = { { 0, 2, 4, 3, 1 }, { 0, 4, 2, 3, 1 } };

	SPKDArray res = spKDArrayCreate(attr, arr, size, &log_msg, &conf_msg);
	printf(
			"finished creating KDArray (note: log and config msgs are left unchecked)\n");
...
int split_test(SPConfig attr) {
	...
	SPKDArray res = spKDArrayCreate(attr, arr, size, &log_msg, &conf_msg);
	printf(
			"finished creating the KDArray (note: log and config msgs are left unchecked)\n");
...
//    printKDArrayMatrix(R);
	...
	size = 3;
	SPKDArray L_sol = spKDArrayCreate(attr, leftArr, size, &log_msg, &conf_msg);
	...
	size = 2;
	SPKDArray R_sol = spKDArrayCreate(attr, rightArr, size, &log_msg,
			&conf_msg);
	...
	int CONT_CHECK = 0;
	printf(
			"using already created splitted arrays L,R - now splitting them again\n");
	...
	size = 2;
	SPKDArray LL_sol = spKDArrayCreate(attr, LL_arr, size, &log_msg, &conf_msg);
	...
	size = 1;
	SPKDArray LR_sol = spKDArrayCreate(attr, LR_arr, size, &log_msg, &conf_msg);
